...
- `growing` — parameterised scaling families generated by default at domain
  sizes `5, 10, ..., 100`. The five original families (`mathcounts`,
  `fourletter`, `tvs`, `workers`, `banana`) stay within the CoLa comparison
  fragment. Three additional families (`nested_choice`, `selected_pairs`,
  `selected_sequence`) exercise dependent configurations that are outside
  that fragment.
- `synthetic` — 384 fixed, stratified synthetic `.cfl` problems saved under
  `problems/benchmarks/synthetic/`. The suite has 16 structural strata,
  three domain-size/difficulty tiers (`8`, `12`, `16`), and eight fixed-seed
  variants per stratum/tier. Eight strata force dependent configurations;
  the other eight balance sets, bags, tuples, sequences, partitions, and
  compositions. Every case has at least one explicit constraint, a non-zero
  non-trivial answer computed by an independent combinatorial formula, and
  tags for its stratum, features, depth, and generation seed.
...
For stable runtime comparisons, run the unchanged benchmark command three
times into separate output directories, then analyze the raw CSV files:

```bash
uv run python -m scripts.benchmarks.analyze \
  --results check-points/run-{1,2,3}/results.csv \
  --timeout 100 \
  --expected-repetitions 3 \
  --reference wfomc \
  --output-dir check-points/analysis
```

The analysis writes `median_results.csv` (one median row per case/backend),
`par2.csv` (failures score `2 * timeout`), and `paired_runtime.csv`. Paired
speedups use only instances solved correctly by both backends in every
repetition; a speedup greater than one means the reference backend is faster.
...
Regenerate that checked-in suite deterministically with:

```bash
uv run python -m scripts.benchmarks.synthetic \
  --seed 20260811 \
  --variants-per-cell 8 \
  --output-dir problems/benchmarks/synthetic
```

Regeneration replaces only `.cfl` files listed by the previous synthetic
manifest; unrelated files in the directory are preserved. The synthetic suite
is intended to measure cross-backend coverage and robustness. Domain scaling
is evaluated separately by the `growing` suite.

"""Planing pipeline — the planning-layer interface for solving problems.
...
@dataclass
class SolveSchedule:
    """Complete solve plan returned by PlaningPipeline.process().
...
def _decompose_into_components(problem: Problem) -> list[Problem]:
    ...
    for c in problem.constraints:
        c_refs = [r for r in constraint_refs(c) if r in all_refs]
    ...
    for root in roots:
        ...
        sub_constraints = tuple(
            ...
            if any(r in comp_refs for r in constraint_refs(c))
        )
...
class PlaningPipeline:
    ...
    def run_passes(
        ...
        *,
        ...
        for pass_spec in pass_classes:
            if isinstance(pass_spec, FixedPointPass):
                cls._run_fixed_point_pass(
                    am,
                    pass_spec,
                    check_compound_invariant=check_compound_invariant,
                )
                continue
            ...
            result = cls._coerce_pass_result(pass_.run(am.problem, am), am.problem)
            if result.changed:
                am.update(result.problem)
    ...
    @classmethod
    def _run_fixed_point_pass(
        cls,
        am: AnalysisManager,
        pass_spec: FixedPointPass,
        *,
        check_compound_invariant: bool = False,
    ) -> None:
        """Run a transform pass until it returns changed=False."""
        pass_ = pass_spec.pass_cls()
        pass_name = pass_spec.pass_cls.__name__
        logger.info("[Pass*] {}", pass_name)

        for iteration in range(1, pass_spec.max_iterations + 1):
            before_compound = (
                sum(1 for c in am.problem.constraints if isinstance(c, _COMPOUND))
                if check_compound_invariant else 0
            )

            logger.debug("[Pass*] {} iteration {}", pass_name, iteration)
            result = cls._coerce_pass_result(pass_.run(am.problem, am), am.problem)
            if not result.changed:
                logger.info(
                    "[Pass*] {} reached fixed point after {} iteration(s)",
                    pass_name,
                    iteration,
                )
                logger.debug("\n{}", fmt_problem(am.problem, stage=f"[After] {pass_name}"))
            ...
            logger.debug(
                "\n{}",
                fmt_problem(am.problem, stage=f"[After] {pass_name} iteration {iteration}"),
            )
        ...
        raise RuntimeError(
            f"{pass_name} did not reach a fixed point after "
            f"{pass_spec.max_iterations} iterations"
        )
    ...
    def _decompose_ordered_sizes(self, problem: Problem) -> list[SolveBranch] | None:
        ...
        for ref, defn in problem.defs:
            if not isinstance(defn, Ordered):
                continue
    ...
    def _make_branch(self, problem: Problem) -> list[SolveBranch]:
        ...
        if pre_analysis.unsatisfiable:
            logger.info("PlaningPipeline: unsatisfiable size constraints (pre-local-passes) → 0")
            return []
        ...
        if len(components) > 1:
            logger.info(
                "PlaningPipeline: decomposed into {} independent components (pre-lowering)",
                len(components),
            )
        ...
        for comp in components:
            ...
            if sub_analysis.unsatisfiable:
                logger.info("PlaningPipeline: component is unsatisfiable → branch contributes 0")
                return []

"""Pass infrastructure and Analysis Manager for the Cofola planing pipeline.
...
class PassResult:
    """Result returned by transform passes that report whether they changed a problem."""
...
class UnsatisfiableConstraint(Exception):
    """Raised by planning passes when a transformed branch is impossible."""


@dataclass(frozen=True)
class FixedPointPass:
    """Pipeline adaptor that repeats a transform pass until it stops changing."""
...
class RefAllocator:
    """Allocate fresh ``ObjRef`` values for transform passes."""

    def __init__(self, problem: Problem) -> None:
        refs = problem.refs()
        self._next_id = max((ref.id for ref in refs), default=-1) + 1

    def new_ref(self) -> ObjRef:
...
class TransformPass(ABC):
    ...
    def run(self, problem: Any, am: Any = None) -> Any:
        ...
        Returns:
            The transformed Problem or a PassResult.
        """
        ...

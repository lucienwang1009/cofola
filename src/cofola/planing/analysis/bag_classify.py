...
from cofola.planing.analysis.entities import AnalysisResult, BagInfo
...
class BagClassification(AnalysisPass):
    ...
    def run(self, problem: Problem, am=None) -> AnalysisResult:
        ...
        for ref in problem.refs():
            ...
            # Partitions/compositions are non-liftable, and their source bag must be dis
            elif isinstance(defn, Grouped):
                non_lifted_refs.add(ref)
                self._mark_dependencies_non_lifted(ref, problem, non_lifted_refs)

            # PartDef inherits the non-liftability of its partition

...
def _encode_bag_choose(
    ...
    # Enforce the exact bag size, whether explicit or inferred from constraints.
...
def _encode_sequence(
    ...
    After LoweringPass:
    - choose without replacement → SetChoose/BagChoose + full SequenceDef
    - choose with replacement from a dynamic set → SetChooseReplace + full SequenceDef
    - choose with replacement from SetInit → position labels over a flatten set
    - full sequences → source size fixed by analysis or size-range decomposition

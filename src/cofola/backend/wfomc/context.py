...
from cofola.backend.wfomc.utils import create_aux_pred, create_cofola_pred, create_cofola_var
...
class Context:
    ...
    def __init__(self, problem: Problem, analysis: AnalysisResult) -> None:
        ...
        Args:
            problem: The immutable planning Problem to encode.
    ...
    def get_leq_pred(self, seq_ref: ObjRef) -> Pred:
        ...
        IMPLEMENTATION:
            ...
            Return leq_pred
        """
        if seq_ref in self.ref2leq_pred:
            return self.ref2leq_pred[seq_ref]
    ...
    def get_predecessor_pred(self, seq_ref: ObjRef) -> Pred:
        ...
        """
        if seq_ref in self.ref2predecessor_pred:
    ...
    def get_next_to_pred(self, seq_ref: ObjRef) -> Pred:
        ...
        IMPLEMENTATION:
            ...
            Return next_to_pred
        """
        if seq_ref in self.ref2next_to_pred:
            return self.ref2next_to_pred[seq_ref]
        ...
        self.ref2next_to_pred[seq_ref] = next_to_pred
        return next_to_pred
    ...
    def get_parts_of(self, partition_ref: ObjRef) -> list[ObjRef]:
        """Return all PartDef ObjRefs belonging to a PartitionDef, sorted by index.

        Args:
            partition_ref: ObjRef of the PartitionDef.

        Returns:
            List of PartDef ObjRefs in ascending index order.

        IMPLEMENTATION:
            from cofola.frontend.objects import PartDef
            parts = [(r, defn.index) for r in problem.refs()
                     if isinstance((defn := problem.get_object(r)), PartDef)
        ...
        for r in self.problem.refs():
            ...
            if isinstance(defn, PartDef) and defn.partition == partition_ref:
    ...
    def prune_evidence(self) -> None:
        ...
        Its ground evidence must survive so typed WFOMC does not drop that
        predicate from the result polynomial.
        """
    ...
    def build(self) -> tuple[EncodedProblem, Decoder]:
        ...
        self.prune_evidence()
        logger.debug(
            "Context.build: evidence (post-prune):\n  {}",
            sorted(str(a) for a in self.unary_evidence),
        )
        new_domain = set()
        ...
        logger.debug(
            "Context.build: Decoder — overcount={}, gen_vars={}, validator={}, indis_vars={}",
            self.overcount,
            self.gen_vars,
            self.validator,
            self.indis_vars,

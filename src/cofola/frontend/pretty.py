...
def _fmt_defn(defn: object, problem: Problem) -> str:
    ...
    match defn:
        ...
        case CompositionDef(source=src, num_parts=np_):
            return f"compose({n(src)}, into={np_})"
        case SetPartDef(partition=p, index=i) | BagPartDef(partition=p, index=i):
            return f"{n(p)}[{i}]"
        case PartPlaceholderDef(partition=p):
            return f"part-of({n(p)})"
        case _:
            return repr(defn)
...
def _fmt_constraint(c: object, problem: Problem) -> str:
    ...
    match c:
        ...
        case OrConstraint(left=l, right=r):
            return f"({_fmt_constraint(l, problem)}) ∨ ({_fmt_constraint(r, problem)})"
        case ForAllParts(partition=p, part_ref=part, constraint_template=tmpl):
            part_s = n(part) if isinstance(part, ObjRef) else "part"
            return f"∀ {part_s} ∈ {n(p)}: {_fmt_constraint(tmpl, problem)}"
        case _:
            return repr(c)

...
def _single_component(source: str) -> tuple[Problem, AnalysisResult]:
    schedule = PlaningPipeline(local_passes=COSO_LOCAL_PASSES).process(parse(source))
...
def test_coso_backend_on_verified_all_json_problem(problem_id: str) -> None:
    pytest.importorskip("coso.launcher")

    import json
    from pathlib import Path

    problems_path = Path(__file__).parent.parent / "problems" / "all.json"

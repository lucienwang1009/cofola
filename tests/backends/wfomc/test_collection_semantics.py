...
def test_small_choice_from_high_multiplicity_bag() -> None:
    ...
    assert parse_and_solve("B = bag(a: 100, b: 100)\nC = choose(B, 2)\n") == 3
...
def test_bag_relations_constrain_singleton_entities(
    ...
    """Bag relations must compare singleton membership and multiplicities."""
    assert parse_and_solve(
        f"B = {source}\nsub = choose(B)\nsup = choose(B)\n{constraint}\n"
    ) == expected
...
def test_singleton_violation_counts_do_not_collide_with_object_names(
    ...
    """An object named like a violation counter must not affect the result."""
...
def test_repeated_bag_inequality_preserves_count(repetitions: int) -> None:
    """Repeating the same inequality must not affect the solution count."""
    assert parse_and_solve(
        "B = bag(a: 1, b: 2)\nsub = choose(B)\nsup = choose(B)\n"
        + "sub != sup\n" * repetitions
    ) == 30
...
def test_bag_difference_excludes_removed_singleton() -> None:
    """A singleton removed by the RHS must not remain in the difference."""
    assert parse_and_solve(
        "B = bag(a: 1)\nX = choose(B)\nY = choose(B)\nZ = X - Y\na not in Z\n"
    ) == 3
